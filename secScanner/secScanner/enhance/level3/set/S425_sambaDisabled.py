...
def S425_sambaDisabled():
    InsertSection("set samba service disabled")
    SET_SAMBA = seconf.get('level3', 'set_samba')
    if SET_SAMBA == 'yes':
        # check status of smb
        service_name = "smb"
        ...
        Display("- Skip set samba service due to config file...", "SKIPPING")
