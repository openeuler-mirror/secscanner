...
def C428_ntalkDisabled():
    InsertSection("check if ntalk service is disabled")
    service_name = "ntalk"
    cmd_audit = "systemctl is-enabled ntalk"
    ...
    if ret == 1 and result in ['disabled', 'masked']:
        ...
        with open(RESULT_FILE, "a") as file:
            file.write("\nC424\n")
        logger.warning("WRN_C428: %s", WRN_C428)
        logger.warning("SUG_C428: %s", SUG_C428)
