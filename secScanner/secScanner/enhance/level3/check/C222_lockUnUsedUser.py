...
def C222_lockUnUsedUser():
    InsertSection("check the unused user")
    unuser_user = seconf.get('level3', 'unused_user_value').split()
    ...
    if counter > 0:
        with open(RESULT_FILE, "a") as file:
            file.write("\nC222\n")
        logger.warning(f"WRN_C222: These users: {error_user} should lock")
        logger.warning("SUG_C222: %s", SUG_C222)
