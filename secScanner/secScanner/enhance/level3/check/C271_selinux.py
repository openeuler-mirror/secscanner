    ...
    if os.path.exists(config_file):
        ...
        if IS_EXIST == 0:
            with open(RESULT_FILE, "a") as file:
                file.write("\nC271\n")
            logger.warning("WRN_C271: %s", WRN_C271)
            logger.warning("SUG_C271: %s", SUG_C271)
        ...
        with open(RESULT_FILE, 'a') as file:
            file.write("\nC271\n")
        logger.warning(f"WRN_C271: {config_file} {WRN_no_file}")
        logger.warning(f"SUG_C271: {config_file} {SUG_no_file}")
