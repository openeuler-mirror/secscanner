...
def C429_rshDisabled():
    InsertSection("check if rsh.socket service is disabled")
    service_name = "rsh.socket"
    cmd_audit = "systemctl is-enabled rsh.socket"
    ...
    if ret == 1 and result in ['disabled', 'masked']:
        ...
        with open(RESULT_FILE, "a") as file:
            file.write("\nC424\n")
        logger.warning("WRN_C429: %s", WRN_C429)
        logger.warning("SUG_C429: %s", SUG_C429)
