        ...
        if ret == 0:
            ...
            if result == 'root':
                ...
                with open(RESULT_FILE, "a") as file:
                    file.write("\nC211\n")
                logger.warning("WRN_C211_01: %s", WRN_C211_01)
                logger.warning("SUG_C211_01: %s", SUG_C211_01)
                Display("- There are users with UID 0 who are not root ...", "WARNING")

        else:
            with open(RESULT_FILE, "a") as file:
                file.write("\nC211\n")
            logger.warning("WRN_C211_02: %s", WRN_C211_02)
            logger.warning("SUG_C211_02: %s", SUG_C211_02)
            Display("- Failed to obtain information with UID 0 ...", "WARNING")
    else:
        with open(RESULT_FILE, "a") as file:
            file.write("\nC211\n")
        logger.warning("WRN_C211_03: %s", WRN_C211_03)
        logger.warning("SUG_C211_03: %s", SUG_C211_03)
