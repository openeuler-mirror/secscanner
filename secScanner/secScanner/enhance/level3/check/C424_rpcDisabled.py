...
def C424_rpcDisabled():
    InsertSection("check if RPC service is disabled")
    service_name = "rpcbind"
    cmd_audit = f"systemctl is-enabled {service_name}"
    ...
    if ret == 1 and result in ['disabled', 'masked']:
        ...
        with open(RESULT_FILE, "a") as file:
            file.write("\nC424\n")
        logger.warning("WRN_C424: %s", WRN_C424)
        logger.warning("SUG_C424: %s", SUG_C424)
