...
def C425_sambaDisabled():
    InsertSection("check if samba service is disabled")
    service_name = "smb"
    cmd_audit = "systemctl is-enabled smb"
    ...
    if ret == 1 and result in ['disabled', 'masked']:
        ...
        with open(RESULT_FILE, "a") as file:
            file.write("\nC425\n")
        logger.warning("WRN_C425: %s", WRN_C425)
        logger.warning("SUG_C425: %s", SUG_C425)
