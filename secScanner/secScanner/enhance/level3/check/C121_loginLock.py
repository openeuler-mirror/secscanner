...
def el67_check_deny():
    ...
    if DENY == '':
        with open(RESULT_FILE, "a") as file:
            file.write("\nC121\n")
        logger.warning("WRN_C121_02: %s", WRN_C121_02)
        logger.warning("SUG_C121: %s", SUG_C121)
        Display("- No user login lock Deny set...", "WARNING")
    elif int(DENY) > 5:
        with open(RESULT_FILE, "a") as file:
            file.write("\nC121\n")
        logger.warning("WRN_C121_01: %s", WRN_C121_01)
        logger.warning("SUG_C121: %s", SUG_C121)
...
def oe_el8_check_deny():
    ...
    if DENY1 == '' and DENY2 == '':
        with open(RESULT_FILE, "a") as file:
            file.write("\nC121\n")
        logger.warning("WRN_C121_02: %s", WRN_C04_02)
...
def C121_loginLock():
    ...
    if OS_ID.lower() in ["centos", "rhel", "redhat", "openeuler", "bclinux"]:
        if OS_DISTRO in ["7", "6"]:
            el67_check_deny()
        elif OS_DISTRO in ["21.10", "20.12", "8", "22.10U1", "22.10", "22.10U2", "22.03", "v24", "24", "21.10U4", "21.10 U4", 'V25']:
            oe_el8_check_deny()
        else:
