...
def C115_passComplex():
    ...
    if not t_minlen.strip().isdigit():
        ...
        with open(RESULT_FILE, "a") as file:
            file.write("\nC115\n")
        logger.warning("WRN_C115_02: %s", WRN_C115_02)
        logger.warning("SUG_C115_01: %s", SUG_C115_01)
        Display("- No Password Minlen set...", "WARNING")
    elif int(t_minlen) < 10:  # '' < '7' but '' cant enter here
        with open(RESULT_FILE, "a") as file:
            file.write("\nC115\n")
        logger.warning("WRN_C115_01: %s", WRN_C115_01)
        logger.warning("SUG_C115_01: %s", SUG_C115_01)
    ...
    if t_minclass == '':
        with open(RESULT_FILE, "a") as file:
            file.write("\nC115\n")
        logger.warning("WRN_C115_04: %s", WRN_C115_04)
        logger.warning("SUG_C115_02: %s", SUG_C115_02)
        Display("- No Password Minclass set...", "WARNING")
    elif int(t_minclass) < 4:  # should >= 4
        with open(RESULT_FILE, "a") as file:
            file.write("\nC115\n")
        logger.warning("WRN_C115_03: %s", WRN_C115_03)
        logger.warning("SUG_C115_02: %s", SUG_C115_02)
    ...
    if n_ucredit == '':
        with open(RESULT_FILE, "a") as file:
            file.write("\nC115\n")
        logger.warning("WRN_C115_06: %s", WRN_C115_06)
        logger.warning("SUG_C115_03: %s", SUG_C115_03)
        Display("- No Password ucredit set...", "WARNING")
    elif n_ucredit > '-1':  # '' < '7' but '' cant enter here
        with open(RESULT_FILE, "a") as file:
            file.write("\nC115\n")
        logger.warning("WRN_C115_05: %s", WRN_C115_05)
        logger.warning("SUG_C115_03: %s", SUG_C115_03)
    ...
    if n_lcredit == '':
        with open(RESULT_FILE, "a") as file:
            file.write("\nC115\n")
        logger.warning("WRN_C115_08: %s", WRN_C115_08)
        logger.warning("SUG_C115_04: %s", SUG_C115_04)
        Display("- No Password lcredit set...", "WARNING")
    elif n_lcredit > '-1':  # '' < '7' but '' cant enter here
        with open(RESULT_FILE, "a") as file:
            file.write("\nC115\n")
        logger.warning("WRN_C115_07: %s", WRN_C115_07)
        logger.warning("SUG_C115_04: %s", SUG_C115_04)
    ...
    if n_dcredit == '':
        with open(RESULT_FILE, "a") as file:
            file.write("\nC115\n")
        logger.warning("WRN_C115_10: %s", WRN_C115_10)
        logger.warning("SUG_C115_05: %s", SUG_C115_05)
        Display("- No Password dcredit set...", "WARNING")
    elif n_dcredit > '-1':  # '' < '7' but '' cant enter here
        with open(RESULT_FILE, "a") as file:
            file.write("\nC115\n")
        logger.warning("WRN_C115_09: %s", WRN_C115_09)
        logger.warning("SUG_C115_05: %s", SUG_C115_05)
    ...
    if n_ocredit == '':
        with open(RESULT_FILE, "a") as file:
            file.write("\nC115\n")
        logger.warning("WRN_C115_12: %s", WRN_C115_12)
        logger.warning("SUG_C115_06: %s", SUG_C115_06)
        Display("- No Password ocredit set...", "WARNING")
    elif n_ocredit > '-1':  # '' < '7' but '' cant enter here
        with open(RESULT_FILE, "a") as file:
            file.write("\nC115\n")
        logger.warning("WRN_C115_11: %s", WRN_C115_11)
        logger.warning("SUG_C115_06: %s", SUG_C115_06)
    ...
    if enforce_root == 'unset':
        with open(RESULT_FILE, "a") as file:
            file.write("\nC115\n")
        logger.warning("WRN_C115_13: %s", WRN_C115_13)
        logger.warning("SUG_C115_07: %s", SUG_C115_07)
