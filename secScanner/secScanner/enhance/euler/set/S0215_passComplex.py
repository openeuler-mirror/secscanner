...
def check_complex_set(keyword, minclass_val, minlen_val, ucredit_val, lcredit_val, dcredit_val, ocredit_val):
    ...
    result = [0, 0, 0, 0, 0, 0, 0]  # return a list as result
        ...
        for line in lines:
            if re.match('password', line) and re.search(keyword, line):
                ...
                if re.search(f'ocredit={ocredit_val}', line):
                    result[5] = 1
                if re.search(f'enforce_for_root', line):
                    result[6] = 1

    if result[0] == 1 and result[1] == 1 and result[2] == 1 and result[3] == 1 \
            and result[4] == 1 and result[5] == 1 and result[6] == 1:
        Display("- Set password complex...", "FINISHED")

def S0215_passComplex():
    InsertSection("set password complex")
    set_password_complex = seconf.get('euler', 'set_passComplex')
    minclass = seconf.get('euler', 'minclass')
    minlen = seconf.get('euler', 'minlen')
    ucredit = seconf.get('euler', 'ucredit')
    lcredit = seconf.get('euler', 'lcredit')
    dcredit = seconf.get('euler', 'dcredit')
    ocredit = seconf.get('euler', 'ocredit')
            ...
            for line in lines:
                if re.match('password', line) and re.search('pam_pwquality.so', line):
                    ...
                    if (re.search(f'minclass={minclass}', line) and re.search(f'minlen={minlen}', line) and
                            ...
                            re.search('enforce_for_root', line)):
