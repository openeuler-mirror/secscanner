...
from secScanner.enhance.level3.check.C222_lockUnUsedUser import C222_lockUnUsedUser

class TestC222_lockUnUsedUser(unittest.TestCase):

    @patch('secScanner.enhance.level3.check.C222_lockUnUsedUser.InsertSection')
    @patch('secScanner.enhance.level3.check.C222_lockUnUsedUser.seconf.get', return_value='adm lp')
    ...
    @patch('secScanner.enhance.level3.check.C222_lockUnUsedUser.open')
    def test_some_users_unlocked(self, mock_open, mock_display, mock_logger, mock_subproc, mock_config, mock_insert):
        # 运行测试的函数
        C222_lockUnUsedUser()

        # 检查预期的警告信息是否已正确记录
        mock_logger.warning.assert_any_call("WRN_C222: These users: ['adm', 'lp'] should lock")
        mock_logger.warning.assert_any_call("SUG_C222: %s", SUG_C222)
