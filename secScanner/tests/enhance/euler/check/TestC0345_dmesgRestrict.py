...
from secScanner.enhance.euler.check.C0345_dmesgRestrict import C0345_dmesgRestrict
import secScanner


class TestC0345_dmesgRestrict(unittest.TestCase):

    @patch('secScanner.enhance.euler.check.C0345_dmesgRestrict.InsertSection')
    @patch('secScanner.enhance.euler.check.C0345_dmesgRestrict.logger')
    @patch('secScanner.enhance.euler.check.C0345_dmesgRestrict.Display')
    ...
    def test_file_not_exists(self, mock_exists, mock_open, mock_display, mock_logger, mock_InsertSection):
        ...
        mock_logger.warning.assert_any_call(f"SUG_C0345: {config_file} {SUG_no_file}")
    ...
    def test_correct_config(self, mock_exists, mock_open, mock_display, mock_logger, mock_InsertSection):
        ...
        C0345_dmesgRestrict()
        mock_InsertSection.assert_any_call("Check set of kernel.dmesg_restrict in sysctl config file")
        mock_logger.info.assert_any_call("Check set of kernel.dmesg_restrict in sysctl config file")
        mock_display.assert_any_call("- Check set of kernel.dmesg_restrict in sysctl config file", "OK")
    ...
    def test_incorrect_config(self, mock_exists, mock_open, mock_display, mock_logger, mock_InsertSection):
        ...
        mock_logger.warning.assert_any_call("SUG_C0345: %s", SUG_C0345)
        mock_display.assert_any_call("- Wrong set of kernel.dmesg_restrict in sysctl config file...", "WARNING")
        mock_open.assert_called_with("result_file_path", "a")
