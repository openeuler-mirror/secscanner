...
class TestC0124_checkRsync(unittest.TestCase):
    ...
    def test_rsync_installed_enabled(self, mock_file, mock_Display, mock_logger, mock_getstatusoutput, mock_InsertSection):
        # 模拟rsync已安装，服务已启用
        mock_getstatusoutput.side_effect = [
            (0, 'rsync-3.1.2-1.el7.x86_64'),
            (0, 'enabled')
        ...
        mock_InsertSection.assert_called_once_with("Check the rsyncd server's status in your Linux System ")
        mock_logger.info.assert_any_call('The status of Rsync is :rsync-3.1.2-1.el7.x86_64')
        mock_logger.warning.assert_any_call("WRN_C0124: %s", WRN_C0124)
        mock_logger.warning.assert_any_call("SUG_C0124: %s", SUG_C0124)
        mock_Display.assert_called_with("- Check the status of rsyncd is enabled...","WARNING")
        mock_file.assert_called_with(RESULT_FILE, 'a+')
