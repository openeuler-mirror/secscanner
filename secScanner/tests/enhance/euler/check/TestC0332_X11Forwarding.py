...
from secScanner.enhance.euler.check.C0332_X11Forwarding import C0332_X11Forwarding
import secScanner


class TestC0332_X11Forwarding(unittest.TestCase):
    ...
    def test_file_not_exists(self, mock_exists, mock_open, mock_display, mock_logger, mock_InsertSection):
        ...
        secScanner.enhance.euler.check.C0332_X11Forwarding.RESULT_FILE = "result_file_path"  # Mock test setup.
        C0332_X11Forwarding()
        mock_InsertSection.assert_any_call("Check set of X11forwarding in sshd config file")
        mock_logger.warning.assert_any_call(f"WRN_C0332: {config_file} {WRN_no_file}")
        mock_logger.warning.assert_any_call(f"SUG_C0332: {config_file} {SUG_no_file}")
    ...
    @patch('secScanner.enhance.euler.check.C0332_X11Forwarding.logger')
    @patch('secScanner.enhance.euler.check.C0332_X11Forwarding.Display')
    @patch('builtins.open', new_callable=mock_open, read_data = "X11Forwarding no\n")
    @patch('os.path.exists')
